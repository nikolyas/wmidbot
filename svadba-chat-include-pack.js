...
$('head').append('<style>#chat_act .message {height:11px!important;} #online-opponents { top:72px!important;} #snd_a_man { position: absolute; left: 8px; top: 144px; z-index: 999; border: solid 1px #ccc; padding: 8px; background: #fff; width: 244px; overflow: hidden;} #snd_a_man a {font-family: tahoma; color: #5685d5;} #snd_a_man a:hove{ text-decoration: none;} #sending_list, #sending_list li { padding:0; margin:0; list-style: none;} #sending_list { border: solid 1px #ccc; overflow: auto; height:90%; } #sending_list li { padding: 2px 5px; border-bottom: solid 1px #ccc;}</style>');
$('#translate').show();
...
if(window.location.host.indexOf('m.svadba.com') > -1){
	...
	$('body').prepend('<div id="chat_act"><b>Активные чаты</b><ul><div align="center" style="padding:10px;">Нет чатов</div></ul></div><div id="count_send"></div><div id="snd_a_man"><a href="#">Отсылка приглашений</a><ul id="sending_list"><span>Нет рассылки</span></ul></div>');
...
setInterval(function(){
	...
	$.getJSON('http://chat.svadba.com/updates/status/everyone/',function(s){
		...
		if(s!=null&&s[0].updates[0].girl.chats.length>0){
			...
			for(c=0;c<s[0].updates[0].girl.chats.length;c++){
				...
				if ((chat_act.length>0&&chat_act.join().search(client_id) == -1)||chat_act.length==0) {
					setTimeout(function(){ 
						if(window.location.hash!='#/'+client_id){
							$.post("http://chat.svadba.com/send-message/"+client_id,{tag:client_id,source:'lc',message:msg},function(ss){ console.log('post'); });
						}
					},30000);
					$('.au').remove();
				...
				if($('#chat_act ul li').size()==0){ $('#chat_act ul').html('');}
					$.getJSON('http://chat.svadba.com/updates/member/'+client_id+'/?member-with='+client_id,function(ssss){ set_mans(ssss); });
				if(window.location.hash=='#/'+client_id){
	...
	$.getJSON('http://chat.svadba.com/updates/unreads/everyone/',function(s){
